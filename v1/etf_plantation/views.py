...
import csv
from v1.etf_plantation import queryset as etf_plantation_queryset
...
class ETFPlantationSymbolImportResource(BaseListResource):
    ...
    def post(self):
        ...
        for data in rows:
            symbol_code = data[0]
            eft_symbol_code_data.append(ETFPlantationSymbols(**{
                 "symbol_code": symbol_code
            }))
            print("symbol_code", symbol_code)

        if eft_symbol_code_data:
                etf_plantation_queryset.delete_etf_plantation_symbols_data(where_condition={"is_active":1})
                etf_plantation_queryset.insert_etf_plantation_symbols_bulk_data(eft_symbol_code_data)

        return success_response(data="")
    

class ETFPlantationSymbolResource(BaseListResource):
    decorators = [authenticated()]
    ...
    def post(self):
        ...
        etf_symbols_objects = etf_plantation_queryset.get_all_etf_plantation_symbols_data(where_query=where_query)
        etf_symbol_list = []
